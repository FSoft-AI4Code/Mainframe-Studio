...
identificationDivision
    : (IDENTIFICATION | ID) DIVISION DOT_FS identificationDivisionBody*?
    ;
...
programIdParagraph
    ...
    )? DOT_FS? identifier? commentEntry?
    ;
...
memorySizeClause
    : MEMORY SIZE? IS? (integerLiteral | cobolWord) (WORDS | CHARACTERS | MODULES)?
    ;
...
defaultComputationalSignClause
    : DEFAULT (COMPUTATIONAL | COMP | DISPLAY)? (SIGN IS?)? (LEADING | TRAILING)? (
        SEPARATE CHARACTER?
    )?
    ;
...
recordKeyClause
    : (RECORD | ACTUAL)? KEY? IS? qualifiedDataName passwordClause? (WITH? DUPLICATES)?
    ;
...
valueOfClause
    : (VALUE | VA) OF? valuePair+
    ;

valuePair
    : systemName IS? (qualifiedDataName | literal) COMMACHAR?
    ;
...
dataBaseDeclare
    : DB qualifiedDataName ((EQUALCHAR | INVOKE)? qualifiedDataName)?
    ;

dataBaseDatasetDeclare
    : integerLiteral (qualifiedDataName | literal) invokeClause? usingClause?
    ;

invokeClause
    : INVOKE (qualifiedDataName | literal)
    ;

usingClause
    : USING (COMMACHAR | qualifiedDataName | literal)*
    ;
...
communicationAttribute
    : cobolWord IS IDENTIFIER
    ;

communicationIoHeader
    : (INPUT | OUTPUT) HEADER IDENTIFIER
    ;

conversationClause
    : CONVERSATION AREA IS IDENTIFIER SIZE IS literal DOT_FS
    ;
...
changeStatement
    : CHANGE ATTRIBUTE (changeFileAttribute | changeLibraryAttribute)
    ;

changeFileAttribute
    : fileAttribute identifier (TO | UP BY | DOWN BY) VALUE? (identifier | literal)
    ;

changeLibraryAttribute
    : libraryAttributeName OF ((literal (BYTITLE | BYFUNCTION)?) | libraryName) TO libraryValueOption
    ;
...
libraryValueOption
    : VALUE? LPARENCHAR? toValueOption RPARENCHAR?
    | literal
    | identifier
    ;
...
createStatement
    : (CREATE | RECREATE) identifier onExceptionClause?
    ;
...
jpEncodingText
    : NC DOUBLEQUOTE (IDENTIFIER+ | JP_TXT+) DOUBLEQUOTE
    | AROBASEEC (MINUSCHAR AROBASEEC)?
    | NC NONNUMERICLITERAL
    ;
...
dataDescriptionEntryFormat1
    : (INTEGERLITERAL | LEVEL_NUMBER_77) (FILLER | NUMERICLITERAL3 | dataName)? (
        dataRedefinesClause
...
dataPictureClause
    : (PICTURE | PIC | PC) IS? pictureString
    ;
...
dataValueClause
    // : ((VALUE | VA) IS? | VALUES ARE? | MINUSCHAR)? dataValueInterval (
    //     COMMACHAR? dataValueInterval
    // )*
    : ((VALUE | VA) IS? | VALUES ARE?) dataValueInterval (COMMACHAR? dataValueInterval)*
    ;
...
procedureDeclarative
    : procedureSectionHeader (DOT | DOT_FS)? useStatement DOT_FS paragraphs
    ;
...
paragraph
    : paragraphName (DOT | DOT_FS) (alteredGoTo | sentence*)
    ;

sentence
    : statement+ (DOT | DOT_FS | NEWLINE)? (commentEntry | COMMENTTAG)?
    ;
...
attachStatement
    : ATTACH sectionName TO eventIdentifier
    ;
...
closePhrase
    : closeFile WITH? (SAVE | PURGE | RELEASE | CRUNCH)?
    ;
...
deleteStatement
    ...
    | DELETE qualifiedDataName onExceptionClause
    ;
...
displayUpon
    : UPON (mnemonicName | environmentName | CONSOLE)
    ;
...
findStatement
    : FIND findOption*? identifier viaClause? (AT condition)? onExceptionClause?
    ;

viaClause
    : VIA findOption? identifier
    ;
...
freeStatement
    : FREE identifier onExceptionClause?
    ;
...
inspectReplacingAllLeading
    : (identifier | literal | figurativeConstant) inspectBy+ inspectBeforeAfter*
    ;

inspectBy
    : BY (identifier | literal | figurativeConstant) COMMACHAR?
    ;
...
lockStatement
    : LOCK (FIRST | NEXT)? qualifiedDataName viaClause? (AT condition)? onExceptionClause?
    ;
...
moveStatement
    : MOVE (ALL | ATTRIBUTE)? (moveToStatement | moveCorrespondingToStatement)
    ;
...
moveToSendingArea
    : (figurativeConstant | identifier | literal | integerLiteral | moveAttributeClause)
    ;
...
moveAttributeClause
    : ATTRIBUTE cobolWord OF ATTRIBUTE EXCEPTIONTASK OF ATTRIBUTE EXCEPTIONTASK OF cobolWord
    ;

// modify statement

modifyStatement
    : MODIFY fileName (LPARENCHAR subscript_ COMMACHAR fileAttribute RPARENCHAR)? TO? modifyTo? (
        AT condition
    )? onExceptionClause?
    ;
...
modifyTo
    ...
    | literal
    | arithmeticExpression
    | ((VALUE | VIA modifyOption?)? mnemonicName inMnemonic?)
    ;
...
openUpdateStatement
    : UPDATE qualifiedDataName onExceptionClause?
    ;
...
openInquiry
    : INQUIRY qualifiedDataName onExceptionClause?
    ;
...
setStatement
    : SET (setToStatement+ | setUpDownByStatement) onExceptionClause?
    ;
...
sortStatement
    : SORT fileName sortOnKeyClause+ sortDuplicatesPhrase? sortCollatingSequencePhrase? sortOptional*
    ;
...
storeStatement
    : STORE qualifiedDataName onExceptionClause?
    ;
...
transactionBegin
    : BEGIN_TRANSACTION (AUDIT | NO_AUDIT)? qualifiedDataName SYNC? onExceptionClause?
    ;
...
transactionEnd
    : END_TRANSACTION (AUDIT | NO_AUDIT)? qualifiedDataName SYNC? onExceptionClause?
    ;
...
useStatement
    : USE (useAfterClause | useDebugClause | useDeadLock | useProcedure)
    ;
...
waitStatement
    : WAIT (AND RESET)? UNTIL? waitArithmeticExpression libraryEntryProcedureUsingClause? libraryEntryProcedureGivingClause?
    ;
...
waitArithmeticExpression
    ...
    | stoqEvent
    | INTERRUPT
    | LPARENCHAR? literal RPARENCHAR?
    ;

attributeChangeEvent
    : ATTRIBUTE CHANGEEVENT IN (identifier | literal)?
    ;

attributeInputEvent
    : ATTRIBUTE INPUTEVENT OF (identifier | literal)
    ;

attributeOutputEvent
    : ATTRIBUTE OUTPUTEVENT ON (identifier | literal)
    ;

attributeAcceptEvent
    : ATTRIBUTE ACCEPTEVENT IN (identifier | literal)
    ;

attributeExceptionEvent
    : ATTRIBUTE EXCEPTIONEVENT (OF | ON)? (identifier | literal)?
    ;
...
crcrEvent
    : (CRCR_INPUT | CRCR_OUTPUT) (identifier | literal)
    ;
...
readOk
    : READ_OK ON (LPARENCHAR (identifier | literal) RPARENCHAR)?
    ;
...
stoqEvent
    : (STOQ_INPUT | STOQ_OUTPUT) identifier
    ;
...
writeAdvancingPhrase
    : (BEFORE | AFTER) (ADVANCING | CHANNEL)? (
        writeAdvancingPage
        | writeAdvancingLines
...
atEndPhrase
    : AT? END (NEXT SENTENCE | statement*)
    ;
...
andOrCondition
    : (AND | OR) (combinableCondition | abbreviation+) //| integerLiteral)
    ;
...
attributeCondition
    : ATTRIBUTE cobolWord OF identifier EQUALCHAR attributeConditionExpr
    ;

attributeConditionExpr
    : VALUE LPARENCHAR? (TRUE | FALSE) RPARENCHAR?
    | literal
    ;
...
relationArithmeticComparison
    : arithmeticExpression relationalOperator (arithmeticExpression | ALL? (SPACE | SPACES))
    ;
...
referenceModifier
    : LPARENCHAR characterPosition COLONCHAR (length | literal)? RPARENCHAR
    ;
...
BEGIN_TRANSACTION
    : B E G I N MINUSCHAR T R A N S A C T I O N
    ;
...
CRCR_INPUT
    : C R C R MINUSCHAR I N P U T
    ;

CRCR_OUTPUT
    : C R C R MINUSCHAR O U T P U T
    ;
...
END_TRANSACTION
    : E N D MINUSCHAR T R A N S A C T I O N
    ;
...
EXCEPTIONEVENT
    : E X C E P T I O N E V E N T
    ;

EXCEPTIONTASK
    : E X C E P T I O N T A S K
    ;
...
ODT_INPUT_PRESENT
    : O D T MINUSCHAR I N P U T MINUSCHAR P R E S E N T
    ;
...
STOQ_INPUT
    : S T O Q MINUSCHAR I N P U T
    ;

STOQ_OUTPUT
    : S T O Q MINUSCHAR O U T P U T
    ;
...
NUMERICLITERAL
    : (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9]* (DOT | COMMACHAR) [0-9]+ (
        ('e' | 'E') (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9]+
    )?
    ;

// NUMERICLITERAL1
//     : (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9Z]* (DOT | COMMACHAR) [0-9Z]+ (
//         ('e' | 'E') (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9Z]+
//     )?
//     ;

// NUMERICLITERAL2
//     : (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? Z* (DOT | COMMACHAR) [0-9Z]+ (
//         ('e' | 'E') (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9]+
//     )?
//     ;
...
NUMERICLITERAL3
    : ((MINUSCHAR | SINGLEQUOTE) (MINUSCHAR | SINGLEQUOTE)* COMMACHAR?)+ [0-9Z]*? MINUSCHAR?
    ;
...
//#MTP Add support comment in line
