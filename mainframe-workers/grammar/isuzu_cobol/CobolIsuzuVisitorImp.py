...
from grammar.isuzu_cobol.CobolIsuzuVisitor import CobolIsuzuVisitor
...
class CobolIsuzuVisitorImp(CobolIsuzuVisitor):
    ...
    def visitIdentificationDivision(
        ...
        programIdParagraph = ctx.programIdParagraph()
        program_id = programIdParagraph.programName().getText()
        self.division_list.append(
            Division(
                division_name="IDENTIFICATION DIVISION",
                start_line=ctx.start.line,
                stop_line=ctx.stop.line,
                section_name_list=[{program_id : program_id}],
            )
        )
        return self.visitChildren(ctx)
