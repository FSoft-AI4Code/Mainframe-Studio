...
author_name
    : (IDENTIFIER | FILENAME | DOT | charDataKeyword | IS | MINUSCHAR | RESET | PICTUREIDENTIFIER)+ (
        LPARENCHAR IDENTIFIER+ RPARENCHAR
    )?
    ;
...
installationParagraph
    : INSTALLATION (DOT_FS | DOT) installationName? commentEntry? (DOT_FS | DOT)?
    ;

installationName
    : (IDENTIFIER | literal | WS | PICTUREIDENTIFIER)+
    ;

// - date written paragraph ----------------------------------

dateWrittenParagraph
    : DATE_WRITTEN (DOT | DOT_FS)? (
        IDENTIFIER
        | literal
        | WS
        | PICTUREIDENTIFIER
...
// - sub schema paragraph ----------------------------------
subSchemaParagraph
    : SUBSCHEMANAME (DOT | DOT_FS) WS? schemaName DOT_FS
    ;
...
specialNamesParagraph
    : SPECIAL_NAMES (DOT_FS | DOT) (specialNameClause+ DOT_FS)?
    ;
...
fileControlEntry
    : ((selectClause fileControlClause*)+ | copyStatement) (DOT_FS | DOT)?
    ;

selectClause
    : SELECT OPTIONAL? fileName (DOT_FS | DOT)?
    ;
...
selectedFunctionClause
    : SELECTED FUNCTION IS? qualifiedDataName
    ;
...
destinationClause
    : (DESTINATION_1 | DESTINATION_2 | DESTINATION_3) IS? (qualifiedDataName | literal)
    ;

formatClause
    : FORMAT IS? qualifiedDataName
    ;

messageModeClause
    : MESSAGE MODE IS? qualifiedDataName
    ;
...
sessionControlClause
    : SESSION CONTROL IS? qualifiedDataName
    ;
...
applyClause
    : APPLY qualifiedDataName (TO fileName+)?
    ;
...
fileSection
    : FILE SECTION DOT_FS (copyStatement DOT_FS?)? fileDescriptionEntry*
    ;

fileDescriptionEntry
    : (FD | SD) fileName
    // (COPY copySource)? 
    (DOT_FS? fileDescriptionEntryClause)* (DOT_FS | DOT)? dataDescriptionEntry*
    ;
...
constantSection
    : CONSTANT SECTION DOT_FS dataDescriptionEntry*
    ;
...
copyStatement
    : COPY copySource ((directoryPhrase | familyPhrase | replacingPhrase | SUPPRESS))* (
        disjoinPhrase joinPhrase
    )? (DOT | DOT_FS)?
    ;

disjoinPhrase
    : DISJOINING IDENTIFIER
    ;

joinPhrase
    : JOINING IDENTIFIER AS? IDENTIFIER (DOT | DOT_FS)?
    ;
...
replaceByStatement
    : REPLACE (NEWLINE* replaceClause)+ (DOT | DOT_FS)
    ;

replaceOffStatement
    : REPLACE OFF (DOT | DOT_FS)
    ;
...
replacement
    : (pseudoText | literal | cobolWord | charDataLine) IDENTIFIER?
    ;
...
jpEncodingText
    : NC DOUBLEQUOTE? (cobolWord* | JP_TXT*) DOUBLEQUOTE?
    | AROBASEEC
    | cobolWord+
    | JP_TXT+
...
dataDescriptionEntryFormat1
    : (INTEGERLITERAL | LEVEL_NUMBER_77) (FILLER | dataName)? (
        ...
        | dataIncludeClause
        | dataCopyClause
        | dataSqlTypeClause // Add more rules
    )* (DOT | DOT_FS)
    ;

dataSqlTypeClause
    : SQL TYPE IS? sqlDataType
    ;

sqlDataType
    : (BINARY | IDENTIFIER) sqlDataLenght
    ;
...
dataIncludeClause
    : INCLUDE (cobolWord | dataName) (REPLACE replaceClause)?
    ;

dataPrintClause
    : PRINTING POSITION IS? integerLiteral
    ;

dataCharacterClause
    : (CHARACTER TYPE IS?)? (MODE_1 | MODE_2 | MODE_3)
    ;
...
procedureDeclaratives
    : DECLARATIVES DOT_FS (procedureDeclarative | copyStatement | paragraph)+ END DECLARATIVES DOT_FS
    ; // CAUTION!!!

procedureDeclarative
    : procedureSectionHeader (DOT | DOT_FS)? useStatement DOT_FS? paragraphs // CAUTIONS!!!
    ;

// sectionExit: sectionName MINUSCHAR EXIT;
...
findStatement
    : FIND (ANY | NEXT | FIRST)? (identifier | literal | RECORD) WITHIN? (identifier | literal)? (
        INDEX
    )? atEndPhrase?
    ;

getStatement
    : GET (ANY | NEXT | FIRST | PRIOR)? (identifier | literal | RECORD) WITHIN? (
        identifier
        | literal
    )? (INDEX | RANGE)? atEndPhrase?
    ;

// getAt: AT (identifier | literal);

// getAny: ANY (identifier | literal) WITHIN (identifier | literal)? (INDEX);

finishStatement
    : FINISH (literal | identifier)?
    ;

eraseStatement
    : ERASE (identifier | literal) (ALL)?
    ;

storeStatement
    : STORE storeSendingArea storeToArea? (PRIOR | NEXT)?
    ;
...
modifyStatement
    : MODIFY (identifier | literal)
    ;

readyStatement
    : READY (literal | identifier)
    ;
...
execCicsStatement2
    : EXEC CICS commandName commandBody? END_EXEC DOT?
    ;
...
commandParameter
    : parameterName (LPARENCHAR (parameterValue | parameterValueWithIndex) RPARENCHAR)?
    ;
...
parameterValueWithIndex
    : parameterNameWithIndex (LPARENCHAR parameterValue RPARENCHAR)
    ;

parameterValue
    : (cobolWord | literal | filename | SCREEN)+
    ;
...
divideByGivingStatement
    : BY (identifier | literal | ZERO) divideGivingPhrase?
    ;
...
evaluateSelect
    : (identifier | literal | arithmeticExpression | condition)+ (
        LPARENCHAR IDENTIFIER ':' IDENTIFIER RPARENCHAR
    )?
    ;
...
goToStatement
    : GO TO? (goToStatementSimple | goToDependingOnStatement) (DOT | DOT_FS)?
    ;
...
ifThen
    : THEN? (NEXT SENTENCE | (statement)+ (DOT | DOT_FS)?)
    ;

ifElse
    : ELSE (NEXT SENTENCE | (statement)+ (DOT | DOT_FS)?)
    ;
...
inspectConvertingPhrase
    : CONVERTING (identifier | literal | figurativeConstant) inspectTo inspectBeforeAfter*
    ;
...
inspectAllLeading
    : (identifier | literal | figurativeConstant) inspectBeforeAfter*
    ;
...
inspectBy
    : BY (identifier | literal | figurativeConstant)
    ;

inspectTo
    : TO (identifier | literal | figurativeConstant)
    ;

inspectBeforeAfter
    : (BEFORE | AFTER) INITIAL? (identifier | literal | figurativeConstant)
    ;
...
// moveOfToStatement: moveOfToSendingArea TO moveOfToReceivingArea+;

// moveOfToSendingArea: (identifier | literal | figurativeConstant) (OF identifier)?;

// moveOfToReceivingArea: identifier (OF identifier)?;
...
performInlineStatement
    : performType? statement* (END_PERFORM | EXIT PERFORM)
    ;

performProcedureStatement
    : procedureName ((THROUGH | THRU) procedureName)? performType? (DOT | DOT_FS)?
    ;
...
transactionStart
    : TRANSACTION START (DOT | DOT_FS)? IDENTIFIER? (DOT | DOT_FS)?
    ;
...
transactionEnd
    : TRANSACTION END (DOT | DOT_FS)?
    ;

transactionCancelStatement
    : TRANSACTION CANCEL IDENTIFIER? (DOT | DOT_FS)?
    ;
...
useStatement
    : USE (useAfterClause | useDebugClause | useDeadLock | useFor)
    ;

useFor
    : FOR qualifiedDataName
    ;
...
figurativeConstant
    : ALL (literal | numericLiteral | SPACES)
    | HIGH_VALUE
    | HIGH_VALUES
    | LOW_VALUE
    | LOW_VALUES
...
DESTINATION_1
    : D E S T I N A T I O N MINUSCHAR '1'
    ;

DESTINATION_2
    : D E S T I N A T I O N MINUSCHAR '2'
    ;

DESTINATION_3
    : D E S T I N A T I O N MINUSCHAR '3'
    ;
...
SUBSCHEMANAME
    : S U B S C H E M A MINUSCHAR N A M E
    ;
...
INTEGERLITERAL
    : (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR | DOT)? [0-9]+
    ;
