...
from parsers.cobol_parser_dnp import parse_cobol_dnp
...
class CobolDNPParserTask(BaseTask):
    def __init__(self):
    ...
    def execute(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with clock:
                parsed_program = parse_cobol_dnp(repository_id, content)
            ...
            save_result = self.reverse_controller.save_reverse_engineering(
                name, repository_id, parsed_program, type=COBOL_TYPE
            )
            ...
            error_msg = f"Error parsing DNP COBOL: {str(e)}"
            logger.error(error_msg)
            return {"status": "error", "message": error_msg}
