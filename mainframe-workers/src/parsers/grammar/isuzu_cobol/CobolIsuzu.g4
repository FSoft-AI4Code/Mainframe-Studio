...
dateWrittenParagraph
    : DATE_WRITTEN (DOT | DOT_FS) (IDENTIFIER | literal | WS | PICTUREIDENTIFIER)* DOT_FS? commentEntry?
    ;
...
formatClause: FORMAT IS? qualifiedDataName;

messageModeClause: MESSAGE MODE IS? qualifiedDataName;
...
applyClause: APPLY qualifiedDataName (TO fileName+)?;
...
copyStatement
    : COPY copySource ((directoryPhrase | familyPhrase | replacingPhrase | SUPPRESS))* (disjoinPhrase joinPhrase)? (DOT | DOT_FS)?
    ;
disjoinPhrase
...
dataDescriptionEntryFormat1
    : (INTEGERLITERAL | LEVEL_NUMBER_77) (FILLER | dataName)? (
        ...
        | dataCharacterClause
        | dataPrintClause // Add more rules
    )* (DOT | DOT_FS)
    ;
...
getStatement
    : GET (ANY | NEXT | FIRST | PRIOR)? (identifier | literal | RECORD) WITHIN? (identifier | literal)? (INDEX | RANGE)? atEndPhrase?;
...
eraseStatement: ERASE (identifier | literal) (ALL)?;

storeStatement: STORE storeSendingArea storeToArea? (PRIOR | NEXT)?;

storeSendingArea: (identifier | literal);

storeToArea: TO (identifier | literal);

// modify statement


modifyStatement: MODIFY (identifier | literal);

readyStatement
    : READY (literal | identifier)
    ;

transactionEndStatement: transactionEnd;
...
readStatement
    : READ fileName NEXT? RECORD? readInto? readWith? readKey? invalidKeyPhrase? notInvalidKeyPhrase? atEndPhrase? notAtEndPhrase? statement* END_READ?
    ;
...
DESTINATION
    ...
    ;
DESTINATION_1: D E S T I N A T I O N MINUSCHAR '1'
    ;

DESTINATION_2: D E S T I N A T I O N MINUSCHAR '2'
    ;

DESTINATION_3: D E S T I N A T I O N MINUSCHAR '3'
    ;
...
INTEGERLITERAL
    : (PLUSCHAR | MINUSCHAR | DOUBLEMINUSCHAR)? [0-9]+
    ;
