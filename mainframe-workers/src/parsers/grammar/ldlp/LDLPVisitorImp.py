...
from parsers.grammar.ldlp.ldlp_schemas import (
    AbortStatement,
    AcceptStatement,
    AccessExtStatement,
    AddStatement,
...
from parsers.grammar.ldlp.LDLPVisitor import LDLPVisitor
...
class LDLPVisitorImp(LDLPVisitor):
    ...
    def assessAdvanceStatement(self, ctx: LDLPParser.AdvanceStatementContext):
        try:
            ...
            logger.warning(f"Invalid numeric literal: {ctx.NUMERIC_LITERALS().getText()}")
        ...
        try:
            ...
            channel = None
            logger.warning(f"Invalid channel number: {ctx.CHANNEL().getText()}")
        output_stream = (
            get_original_code_content(
                ctx.parser,
