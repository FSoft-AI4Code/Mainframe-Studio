...
programUnit
    : identificationDivision environmentDivision? dataDivision?
...
copyStatement
    : COPY copySource ((directoryPhrase | familyPhrase | replacingPhrase | SUPPRESS))* (disjoinPhrase joinPhrase)?
    ;
disjoinPhrase
...
charDataSql
    : 
    (charDataLine | COPY | REPLACE | NEWLINE | CURSOR | SELECT | FROM | FOR | OPEN | CLOSE | PAGE |VALUES| SET|SECURITY| TOP | COUNT | ORDER |WITH|POSITION |NEXT| VALUE|REMARKS|PLUSCHAR | FILLER|BLOCK|USAGE|NUMERICLITERAL2|OR|FILENAME| INDEX|NULL_|CODE|IN|TYPE|KEY|BEFORE|FIRST|STATUS|AS| DATE|SCREEN|TO|WHEN|SUM|RD|LINE|NOT|LEFT|RIGHT|THEN|ELSE|END|CLASS|relationalOperator | WS | IS | ID | LOCK | TABLE | EXCLUSIVE | MODE | MERGE | GROUP | GLOBAL|USING|DAY|REFERENCE | REFERENCES | EOL | MMDDYYYY | DIVISION | CALL | DOUBLE | INTEGER | DETAIL | MESSAGE | LD | LAST | LIMIT | SLASHCHAR | DESTINATION | SORT | SIGN | COL|COMP | CHANNEL | UP | PROCEDURE | PASSWORD | PIPECHAR | UNDERSCORECHAR | DEFAULT | UNIT | CD | RELEASE | BOTTOM | DOLLARCHAR | FUNCTION | YEAR | EXPORT | BIT | PIC | OTHER | SEGMENT | STANDARD | CHANGED | INSTALLATION | DEFINITION | REQUIRED)+
...
cobolWord
    : IDENTIFIER
    | charDataKeyword
    // | IDENTIFIERWITHHASHANDCOLON;
    ;
// IDENTIFIERWITHHASHANDCOLON
//     : COLONCHAR IDENTIFIER COLONCHAR MINUSCHAR IDENTIFIER | IDENTIFIER HASH IDENTIFIER | LPARENCHAR IDENTIFIER RPARENCHAR MINUSCHAR IDENTIFIER;
...
dataDescriptionEntryFormat1
    ...
    ;
dataSqlTypeClause:  SQL TYPE IS? sqlDataType;
sqlDataType: (BINARY | IDENTIFIER | IDENTIFIER VARYING | INTEGER) sqlDataLenght?;
sqlDataLenght: LPARENCHAR (integerLiteral | IDENTIFIER) RPARENCHAR; 
dataCharacterClause
    : (CHARACTER TYPE IS)? MODE_1   ;
...
procedureDeclaratives
    : DECLARATIVES DOT_FS (procedureDeclarative+ | copyStatement) END DECLARATIVES DOT_FS
    ;
...
xmlParseStatement: XML PARSE xmlDataname xmlProcessingProcedure endXml?;

xmlDataname: dataName;

xmlProcessingProcedure: PROCESSING PROCEDURE IS? procedureName ((THRU | THROUGH) procedureName)?;
endXml: END_XML;
 execSqlStatement2: EXEC SQL charDataSql END_EXEC;   
...
displayStatement
    : DISPLAY displayOperand+ displayAt? displayUpon? displayWith? (DOT_FS | DOT)?
    ;
...
evaluateSelect
    ...
    | condition)+ (LPARENCHAR IDENTIFIER ':' IDENTIFIER RPARENCHAR)?
    ;
...
performInlineStatement
    : performType? statement* (END_PERFORM | EXIT PERFORM '/n' END_IF)
    ;
...
WS
    : [ \t\f;]+ -> channel(HIDDEN)
    ;
WS_JP: '@' -> channel(HIDDEN)
    ;

WS_JP_ENCODED: 'ã€€' -> channel(HIDDEN)
    ;
