...
from parsers.grammar.dnp_cobol.dnp_helpers import extract_cobol_variable
from parsers.grammar.dnp_cobol.DNPParser import DNPParser
from parsers.grammar.dnp_cobol.DNPVisitor import DNPVisitor
...
class DNPVisitorImp(DNPVisitor):
    ...
    def __init__(self, parser: DNPParser):
        ...
        self.statements = []
        self.system_database_list: List[Database] = []
        self.func = {
    ...
    def visitFileControlEntry(self, ctx: DNPParser.FileControlEntryContext):
        ...
        file_control_clause_list: List[DNPParser.FileControlClauseContext] = (
            ctx.fileControlClause()
        )

        assignment_name = None
        for file_control_clause in file_control_clause_list:
            assign_clause: DNPParser.AssignClauseContext = (
        ...
        access_mode_clause = recursive_find_first_child_by_type(
            ctx, DNPParser.AccessModeClauseContext
        )
        organization_clause = recursive_find_first_child_by_type(
            ctx, DNPParser.OrganizationClauseContext
        )
    ...
    def visitDataBaseSectionEntry(self, ctx: DNPParser.DataBaseSectionEntryContext):
        if ctx.dataBaseDeclare():
            system_database_declare_ctx = ctx.dataBaseDeclare()

            system_database_name = get_original_code_content(
                self.parser,
                system_database_declare_ctx.qualifiedDataName(0).start.tokenIndex,
                system_database_declare_ctx.qualifiedDataName(0).stop.tokenIndex,
            )

            invoke_name = (
                get_original_code_content(
                    self.parser,
                    system_database_declare_ctx.qualifiedDataName(1).start.tokenIndex,
                    system_database_declare_ctx.qualifiedDataName(1).stop.tokenIndex,
                )
                if system_database_declare_ctx.qualifiedDataName(1)
                else None
            )

            self.system_database_list.append(
                Database(
                    database_name=system_database_name,
                    invoke_name=invoke_name,
                    tables=[],
                )
            )

        if ctx.dataBaseDatasetDeclare():
            database_declare_ctx = ctx.dataBaseDatasetDeclare()

            database_name = get_original_code_content(
                self.parser,
                database_declare_ctx.getChild(1).start.tokenIndex,
                database_declare_ctx.getChild(1).stop.tokenIndex,
            )

            invoke_name = (
                get_original_code_content(
                    self.parser,
                    database_declare_ctx.invokeClause().getChild(1).start.tokenIndex,
                    database_declare_ctx.invokeClause().getChild(1).stop.tokenIndex,
                )
                if database_declare_ctx.invokeClause()
                else None
            )

            using_clause_ctx = database_declare_ctx.usingClause()
            ...
            if using_clause_ctx:
                using_names = [
                    get_original_code_content(
                        self.parser,
                        using_name.start.tokenIndex,
                        using_name.stop.tokenIndex,
                    )
                    for using_name in using_clause_ctx.qualifiedDataName()
                ]

            self.system_database_list[-1].tables.append(
                DatabaseTable(
    ...
    def visitWorkingStorageSection(self, ctx: DNPParser.WorkingStorageSectionContext):
        data_description_entry_list: List[DNPParser.DataDescriptionEntryContext] = (
        ...
        for data_description_entry in data_description_entry_list:
            ...
            # Check if it's a COPY statement
            if isinstance(entry, DNPParser.CopyStatementContext):
                copy_source = recursive_find_first_child_by_type(
                    entry, DNPParser.CopySourceContext
                )
    ...
    def visitLinkageSection(self, ctx: DNPParser.LinkageSectionContext):
        ...
        for data_description_entry in data_description_entry_list:
            if isinstance(
                data_description_entry.getChild(0), DNPParser.CopyStatementContext
            ):
                continue
            variable = extract_cobol_variable(data_description_entry.getChild(0))
    ...
    def visitOpenInputStatement(self, ctx: DNPParser.OpenInputStatementContext):
        open_input_list: List[DNPParser.OpenInputContext] = ctx.openInput()
        for open_input in open_input_list:
            file_control_name = recursive_find_first_child_by_type(
                open_input, DNPParser.FileNameContext
            )
    ...
    def visitOpenOutputStatement(self, ctx: DNPParser.OpenOutputStatementContext):
        open_output_list: List[DNPParser.OpenOutputContext] = ctx.openOutput()

        for open_output in open_output_list:
            file_control_name = recursive_find_first_child_by_type(
                open_output, DNPParser.FileNameContext
            )
    ...
    def visitOpenIOStatement(self, ctx: DNPParser.OpenIOStatementContext):
        file_name_list: List[DNPParser.FileNameContext] = ctx.fileName()

        for file_name in file_name_list:
            file_control_name = recursive_find_first_child_by_type(
                file_name, DNPParser.FileNameContext
            )
