...
class ExecSqlStatement2(Statement):
    """EXEC SQL statement in COBOL"""
...
class EvaluateStatement(Statement):
    ...
    evaluateWhenOther: SerializeAsAny[List[Statement]] = []
...
class MultiplyStatement(Statement):
    ...
    multiplier_identifier: Optional[str] = None
    multiplier_literal: Optional[str] = None
    multiplicand_identifiers: List[str] = []
    multiplicand_literals: List[str] = []
    results: List[str] = []
...
class ReleaseStatement(Statement):
    """Release statement in COBOL"""

    record_name: str
    from_data_name: Optional[str] = None


class ReturnStatement(Statement):
    """Return statement in COBOL"""

    file_name: str
    into_data_name: Optional[str] = None
    at_end_phrase: SerializeAsAny[List[Statement]] = []
    not_at_end_phrase: SerializeAsAny[List[Statement]] = []
...
class SortKey(BaseModel):
    keys: List[str] = []
    sort_type: Literal["ASCENDING", "DESCENDING"]
...
class SortStatement(Statement):
    ...
    through_procedure_name: Optional[str] = None
...
class UnstringDelimitedBy(BaseModel):
    literal: Optional[str] = None
    identifier: Optional[str] = None
    is_all: bool = False


class UnstringInto(BaseModel):
    identifier: str
    delimiter: Optional[str] = None
    count: Optional[str] = None


class UnstringStatement(Statement):
    """Unstring statement in COBOL"""

    identifier: str
    delimited_by_list: List[UnstringDelimitedBy] = []
    into: List[UnstringInto] = []
...
class IBMStatementFactory:
    ...
    def from_dict(data: dict) -> Statement:
        ...
        schema_dict = {
            ...
            "ExecSqlStatement2": ExecSqlStatement2,
        ...
        if isinstance(statement, AcceptStatement):
            ...
            for i, statement_dict in enumerate(on_exception_clause):
                statement.on_exception_clause[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
            not_on_exception_clause = data.get("not_on_exception_clause", [])
            for i, statement_dict in enumerate(not_on_exception_clause):
                statement.not_on_exception_clause[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
        ...
        if isinstance(statement, DivideStatement):
            ...
            for i, statement_dict in enumerate(on_size_error_phrase):
                statement.on_size_error_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
            not_on_size_error_phrase = data.get("not_on_size_error_phrase", [])
            for i, statement_dict in enumerate(not_on_size_error_phrase):
                statement.not_on_size_error_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
        ...
        if isinstance(statement, EvaluateStatement):
            ...
            evaluate_when_other = data.get("evaluateWhenOther", [])
            for i, statement_dict in enumerate(evaluate_when_other):
                statement.evaluateWhenOther[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
        ...
        if isinstance(statement, SearchStatement):
            when = data.get("when", [])
            for i, statement_dict in enumerate(when):
                statement.when[i].statements.append(
                    IBMStatementFactory.from_dict(statement_dict)
                )
            at_end_phrase = data.get("at_end_phrase", [])
            for i, statement_dict in enumerate(at_end_phrase):
        ...
        if isinstance(statement, StartStatement):
            invalid_key_phrase = data.get("invalid_key_phrase", [])
            for i, statement_dict in enumerate(invalid_key_phrase):
                statement.invalid_key_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
        ...
        if isinstance(statement, StringStatement):
            ...
            for i, statement_dict in enumerate(on_overflow_phrase):
                statement.on_overflow_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
            not_on_overflow_phrase = data.get("not_on_overflow_phrase", [])
            for i, statement_dict in enumerate(not_on_overflow_phrase):
                statement.not_on_overflow_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
                )
        ...
        if isinstance(statement, UnstringStatement):
            on_overflow_phrase = data.get("on_overflow_phrase", [])
            for i, statement_dict in enumerate(on_overflow_phrase):
                statement.on_overflow_phrase[i] = IBMStatementFactory.from_dict(
                    statement_dict
