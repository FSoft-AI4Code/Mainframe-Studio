...
from .constants import FileType, SystemType
...
class RuleBasedTextClassifierBySystem:
    ...
    def __init__(self):
        ...
        verbs_file = os.path.join(project_root, settings.COBOL_VERBS_FILE)
    ...
    def is_copy(self, code: str, encoding: str) -> bool:
        ...
        # Pattern for typical copybook field definitions
        pattern = re.compile(
            r"^[A-Za-z0-9\s]{6}[\s\*]\s*(\d{2})\s+([a-zA-Z][a-zA-Z0-9\-]+|[A-Za-z0-9\-!@#$%'\"^&*()_=+;:{}\[\],\.|]{2,})(\s+(COMP\s+)?(PICTURE|PIC|REDEFINES))?",
            re.IGNORECASE,
        )
    ...
    def is_proc(self, code: str) -> bool:
        ...
        for line in lines:
            if re.match(r"\/\/REPROC\s+PROC", line):
                return True
        return False
